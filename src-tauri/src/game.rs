...
pub async fn install_build(
  ...
  let result = tauri::async_runtime::spawn_blocking(move || {
    install_build_blocking(&mode_name_owned, task_progress_state)
  })
    .await
    .map_err(|error| format!("Failed to complete the install task: {error}"))?;
...
fn install_build_blocking(
  ...
  install_or_update_build(&tbw_root, mode_name, &source, Some(&progress_state))
}
...
fn ensure_mode_build_current(
  ...
  if let Some(source) = try_find_build_source(&tbw_root, mode_name)? {
    if build_source_has_remote_download(&source) {
      let _ = install_or_update_build(&tbw_root, mode_name, &source, Some(&progress_state))?;
    }
  }
    ...
    let archive_bytes = download_bytes_with_progress(
      ...
      Some(|downloaded_bytes, total_bytes| {
        let progress_percent = match total_bytes {
          Some(total) if total > 0 => map_fraction_to_progress(downloaded_bytes, total, 8, 72),
          _ => 18,
        };
        let stage_text = match total_bytes {
          ...
          _ => format!("Загрузка архива... {}", format_compact_bytes(downloaded_bytes)),
        };
...
fn try_find_build_source(tbw_root: &Path, mode_name: &str) -> Result<Option<BuildSourceEntry>, String> {
  let manifest_path = tbw_root.join("build_sources.json");
...
fn resolve_static_build_download(source: &BuildSourceEntry) -> Result<ResolvedBuildDownload, String> {
...
fn is_build_installed_in_versions(versions_dir: &Path, mode_name: &str) -> Result<bool, String> {
  ...
  let entries = fs::read_dir(versions_dir)
    .map_err(|error| format!("Failed to read directory {}: {error}", versions_dir.display()))?;
  ...
  F: FnMut(usize, usize),
{
  ...
  for index in 0..total_entries {
    ...
    fs::write(&output_path, buffer)
      .map_err(|error| format!("Failed to write extracted file {}: {error}", output_path.display()))?;
...
fn resolve_extracted_build_root(extract_root: &Path) -> Result<PathBuf, String> {
  let entries = fs::read_dir(extract_root)
    .map_err(|error| format!("Failed to read directory {}: {error}", extract_root.display()))?;
...
fn move_directory_contents(source_root: &Path, target_dir: &Path) -> Result<(), String> {
  let entries = fs::read_dir(source_root)
    .map_err(|error| format!("Failed to read directory {}: {error}", source_root.display()))?;
...
#[cfg(windows)]
fn build_debug_powershell_script(
  java_executable: &OsStr,
  args: &[OsString],
) -> String {
...
fn build_java_arguments(
  ...
  if plan.jvm_argument_tokens.is_empty() {
    ...
    args.extend(plan.jvm_argument_tokens.iter().map(|token| {
      OsString::from(apply_argument_replacements(token, plan, None))
    }));
...
fn apply_argument_replacements(
  token: &str,
  plan: &LaunchPlan,
  nickname: Option<&str>,
) -> String {
...
fn rule_matches_features(rule: &serde_json::Value) -> bool {
  ...
  features.iter().all(|(_, value)| value.as_bool().is_none_or(|flag| !flag))
}

fn resolve_launch_plan(mode_name: &str, preferred_game_version: Option<&str>) -> Result<LaunchPlan, String> {
  let tbw_root = find_tbw_root()?;
  ...
  let inherited_version =
    ensure_inherited_manifest_available(&tbw_root, &versions_dir, launch_manifest, &http_client)?;
  ...
  let asset_index_name =
    ensure_asset_index_and_objects(&assets_dir, client_manifest, &http_client)?;
  let logging_argument = ensure_logging_config_argument(&assets_dir, client_manifest, &http_client)?;
...
fn resolve_manifest_library_paths(
  ...
  if let Some(parent) = inherited_manifest {
    collect_manifest_library_paths(libraries_dir, parent, &mut merged, http_client)?;
  }

  collect_manifest_library_paths(libraries_dir, manifest, &mut merged, http_client)?;
...
fn collect_manifest_library_paths(
  ...
  for library in libraries {
    ...
    ensure_library_artifact(library, library_name, relative_path, &jar_path, http_client)?;

    let key = library_merge_key(library)
      .unwrap_or_else(|| relative_path.to_string());

    if let Some(index) = out.iter().position(|(existing_key, _)| *existing_key == key) {
...
fn ensure_library_artifact(
    ...
    .is_some_and(|name| name.starts_with("net.minecraftforge:forge:"))
  {
    if let Some(universal_relative_path) = forge_universal_relative_path(relative_path) {
      let universal_url = format!("https://maven.minecraftforge.net/{universal_relative_path}");
  ...
  for url in candidate_urls {
    match download_to_path(http_client, &url, jar_path, expected_sha1, expected_size) {
      Ok(()) => return Ok(()),
...
fn ensure_asset_index_and_objects(
  ...
  ensure_asset_objects(&asset_index_path, &objects_dir, http_client)?;
...
fn ensure_asset_objects(
  ...
  let index_raw = fs::read_to_string(asset_index_path)
    .map_err(|error| format!("Failed to read asset index {}: {error}", asset_index_path.display()))?;
  let index_json: serde_json::Value = serde_json::from_str(&index_raw)
    .map_err(|error| format!("Asset index {} is invalid: {error}", asset_index_path.display()))?;
  let objects = index_json["objects"]
    .as_object()
    .ok_or_else(|| format!("Asset index {} does not contain objects.", asset_index_path.display()))?;
...
fn resolve_asset_index_metadata(
  ...
  Ok((asset_index_name, asset_index_url, asset_index_sha1, asset_index_size))
}
...
fn ensure_inherited_manifest_available(
  ...
  ensure_version_manifest_available(tbw_root, versions_dir, parent_version_id, http_client)
    .map(Some)
}
...
fn ensure_version_manifest_available(
  ...
  download_to_path(http_client, &manifest_url, &manifest_path, None, None).map_err(|error| {
    format!("Failed to download version manifest {version_id}: {error}")
  })?;
...
fn ensure_client_jar_available(
  ...
  let client_url = manifest["downloads"]["client"]["url"]
    .as_str()
    .filter(|value| !value.is_empty())
    .ok_or_else(|| format!("The version manifest {version_name} does not contain a client jar URL."))?;
  let client_sha1 = manifest["downloads"]["client"]["sha1"].as_str();
  let client_size = manifest["downloads"]["client"]["size"].as_u64();

  download_to_path(http_client, client_url, &client_jar, client_sha1, client_size).map_err(
    |error| format!("Failed to download client jar {version_name}: {error}"),
  )?;
...
fn ensure_natives_ready(
  ...
  let target_dir = resolve_natives_dir(natives_root, launch_version_name, version_id, base_version_id)?;
  ensure_directory_ready(&target_dir)?;

  if let Some(parent) = inherited_manifest {
    extract_manifest_natives(libraries_dir, &target_dir, parent, http_client)?;
  }

  extract_manifest_natives(libraries_dir, &target_dir, manifest, http_client)?;
...
fn find_matching_natives_dir(
  ...
  let entries = fs::read_dir(natives_root)
    .map_err(|error| format!("Failed to read directory {}: {error}", natives_root.display()))?;
...
fn extract_manifest_natives(
  ...
  for library in libraries {
    ...
    if let Some(index) = merged.iter().position(|(existing_key, _)| *existing_key == key) {
  ...
  for (_, library) in merged {
    ensure_native_classifier_extracted(libraries_dir, target_dir, &library, http_client)?;
  }
...
fn library_has_natives_for_current_os(library: &serde_json::Value) -> bool {
  ...
  library["natives"].get(os_name).and_then(|value| value.as_str()).is_some()
}
...
fn ensure_native_classifier_extracted(
  ...
  let os_name = current_minecraft_os_name();
  let classifier_key = library["natives"][os_name]
    .as_str()
    .ok_or_else(|| "The native library entry is missing the current OS classifier.".to_string())?;
  ...
  if !classifier_path.is_file() {
    ...
    .map_err(|error| format!("Failed to download native classifier {classifier_key}: {error}"))?;
  }

  extract_native_archive(&classifier_path, target_dir, library)
}
...
fn extract_native_archive(
  ...
  let archive_bytes = fs::read(archive_path)
    .map_err(|error| format!("Failed to read native archive {}: {error}", archive_path.display()))?;
  let cursor = Cursor::new(archive_bytes);
  let mut archive = ZipArchive::new(cursor)
    .map_err(|error| format!("Failed to open native archive {}: {error}", archive_path.display()))?;
  ...
  for index in 0..archive.len() {
    let mut file = archive
      .by_index(index)
      .map_err(|error| format!("Failed to read native archive entry in {}: {error}", archive_path.display()))?;
    let entry_name = file.name().replace('\\', "/");
    ...
    fs::write(&output_path, buffer)
      .map_err(|error| format!("Failed to write extracted native file {}: {error}", output_path.display()))?;
  }
...
fn find_version_manifest_url(
  ...
  let versions = global_manifest["versions"]
    .as_array()
    .ok_or_else(|| "The global version manifest does not contain a versions list.".to_string())?;
...
fn download_to_path(
  ...
  let bytes = download_bytes(http_client, url)?;
...
fn download_bytes(http_client: &Client, url: &str) -> Result<Vec<u8>, String> {
  download_bytes_with_progress(http_client, url, Option::<fn(u64, Option<u64>)>::None)
}
...
fn read_manifest_file(
  ...
  let manifest_raw = fs::read_to_string(manifest_path)
    .map_err(|error| format!("Failed to read version manifest {requested_version_id}: {error}"))?;

  serde_json::from_str(&manifest_raw).map_err(|error| {
    format!(
      "Version manifest {requested_version_id} is invalid: {error}"
    )
  })
}
...
fn resolve_game_dir(
  ...
  ensure_directory_exists(
    versions_dir,
    "The .tbw/versions directory is missing.",
  )?;
  ...
  for entry in entries {
    let entry = entry
      .map_err(|error| format!("Failed to read directory entry in {}: {error}", versions_dir.display()))?;
    let path = entry.path();
    if !path.is_dir() {
      continue;
    }
  ...
  Err(format!("Build \"{mode_name}\" was not found in .tbw/versions."))
}
...
fn collect_named_files(
  ...
  for entry in entries {
    let entry = entry
      .map_err(|error| format!("Не удалось прочитать элемент папки {}: {error}", dir.display()))?;
    let path = entry.path();
